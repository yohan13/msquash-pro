...
import { getConfig, getDay, createBooking, deleteBooking, createBlock, deleteBlock, exportCSV } from '../api'
...
export default function Book() {
  ...
  async function handleReserve(courtId, time) {
    ...
    try {
      await createBooking({ date, time, courtId, note: note.trim() || null })
      await loadDay()
      setRefresh(r => r + 1)
      setBanner('success', `Court ${courtId} réservé à ${time} ✓`)
    } catch (e) {
      const msgs = {
  ...
  async function handleExport() {
    ...
    } catch (e) { setBanner('error', 'Erreur lors de l\'export.') }
  }
    ...
    <main className="max-w-7xl mx-auto px-4 py-6 space-y-5">
      ...
      <section className="card">
        ...
        <div className="card-body grid md:grid-cols-2 gap-4">
          ...
          <div className="md:col-span-2 text-xs text-ink-muted bg-gray-50 rounded-xl px-3 py-2">
        ...
        <div className="card-header flex flex-wrap items-center justify-between gap-3">
          ...
          {isAdmin && (
            <div className="toolbar">
              ...
              <button type="button" className="btn btn-outline text-sm" onClick={handleExport}>
                ⬇ CSV
              </button>
            </div>
          )}
